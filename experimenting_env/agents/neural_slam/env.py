...
@baseline_registry.register_env(name="ExpSlam-v0")
class Exploration_Env(habitat.RLEnv):
    def __init__(self, config, args=None, rank=10, *other, **kwargs):
        super().__init__(
            config,
            **kwargs,
        )
        ...
        #     open("data/noise_models/sensor_noise_fwd.pkl", 'rb')
        # )
        # self.sensor_noise_right = pickle.load(
        #     open("data/noise_models/sensor_noise_right.pkl", 'rb')
        # )
        # self.sensor_noise_left = pickle.load(
        #     open("data/noise_models/sensor_noise_left.pkl", 'rb')
        # )
    ...
    def save_trajectory_data(self):
        if "replica" in self.scene_name:
            folder = (
                self.args.save_trajectory_data
                + "/"
    ...
    def save_position(self):
        ...
        self.trajectory_states.append(
            [self.agent_state.position, self.agent_state.rotation]
        )
    ...
    def reset(self):
        ...
        mapper_gt_pose = (
            self.curr_loc_gt[0] * 100.0,
            self.curr_loc_gt[1] * 100.0,
            np.deg2rad(self.curr_loc_gt[2]),
        )

        # Update ground_truth map and explored area
        fp_proj, self.map, fp_explored, self.explored_map = self.mapper.update_map(
            depth, mapper_gt_pose
        )
    ...
    def step(self, action):
        ...
        if action == 2:  # Forward
            ...
            print(f"Something strange happened with action {action}")
            noisy_action = 1
        ...
        self.curr_loc = pu.get_new_pose(self.curr_loc, (dx_base, dy_base, do_base))
        ...
        self.info['pose_err'] = [dx_gt - dx_base, dy_gt - dy_base, do_gt - do_base]
    ...
    def get_global_reward(self):
        ...
        m_reward = (curr_explored_area - self.prev_explored_area) * 1.0
        m_ratio = m_reward / reward_scale
        m_reward = m_reward * 25.0 / 10000.0  # converting to m^2
    ...
    def build_mapper(self):
        ...
        self.selem = skimage.morphology.disk(
            self.args.obstacle_boundary / self.args.map_resolution
        )
    ...
    def get_short_term_goal(self, inputs):
        ...
        relative_dist = relative_dist * 5.0 / 100.0
        ...
        output[0] = int((relative_angle % 360.0) / 5.0)
        ...
        #         self.map[gx1:gx2, gy1:gy2] * self.explored_map[gx1:gx2, gy1:gy2],
        #     )
    ...
    def _get_stg(self, grid, explored, start, goal, planning_window):
        ...
        if (
            ...
            traversible[
                int(goal[0] - x1) - 2 : int(goal[0] - x1) + 3,
                int(goal[1] - y1) - 2 : int(goal[1] - y1) + 3,
            ] = 1
        else:
        ...
        while not path_found:
            traversible = (
                skimage.morphology.binary_dilation(
                    grid[gx1:gx2, gy1:gy2][x1:x2, y1:y2], self.selem
                )
                != True
            )
            traversible[self.visited[gx1:gx2, gy1:gy2][x1:x2, y1:y2] == 1] = 1
            ...
            traversible[
                int(goal[0] - x1) - goal_r : int(goal[0] - x1) + goal_r + 1,
            ...
            for i in range(1):
                stg_x_gt, stg_y_gt, replan = planner.get_short_term_goal(
                    [stg_x_gt, stg_y_gt]
                )

            if replan and buf < 100.0:
                buf = 2 * buf
                x1 = max(0, int(x1 - buf))
                x2 = min(grid.shape[0], int(x2 + buf))
                y1 = max(0, int(y1 - buf))
