...
from experimenting_env.agents.baselines import BaseRLTrainer
...
from .utils import GlobalRolloutStorage


@baseline_registry.register_trainer(name="neuralslam")
class NeuralSlam(BaseRLTrainer):
    def __init__(self, config, exp_base_dir, **kwargs):
        super().__init__(config)

        self.config = config
        self.args = config.NEURALSLAM
        self.kwargs = {'args': self.args}
    ...
    def _init_train(self):
        ...
        args.num_processes = envs.num_envs
        args.max_episode_length = self.config.ENVIRONMENT.MAX_EPISODE_STEPS
        args.freeze()
        device = args.device
        map_size = args.map_size_cm // args.map_resolution
        full_w, full_h = map_size, map_size
        local_w, local_h = int(full_w / args.global_downscaling), int(
            full_h / args.global_downscaling
        )
        ...
        l_observation_space = gym.spaces.Box(
            0, 255, (3, self.frame_width, self.frame_width), dtype="uint8"
        )
        ...
        self.nslam_module = Neural_SLAM_Module(args).to(device)
        # Local policy
        self.l_policy = Local_IL_Policy(
            l_observation_space.shape,
            3,  # envs.action_space.n,
            recurrent=args.use_recurrent_local,
            hidden_size=args.local_hidden_size,
            deterministic=args.use_deterministic_local,
        ).to(device)
        ...
        state_dict = torch.load(
            args.load_nslam, map_location=lambda storage, loc: storage
        )

        self.nslam_module.load_state_dict(state_dict)

        state_dict = torch.load(
            args.load_local, map_location=lambda storage, loc: storage
        )
        self.l_policy.load_state_dict(state_dict)

        state_dict = torch.load(
            args.load_global, map_location=lambda storage, loc: storage
        )
    ...
    def generate(self):
        ...
        poses = (
            torch.from_numpy(
                np.asarray(
                    [obs[env_idx][1]["sensor_pose"] for env_idx in range(num_scenes)]
                )
            )
            .float()
            .to(device)
        ...
        def init_map_and_pose():
            ...
            for e in range(num_scenes):
                ...
                lmb[e] = get_local_map_boundaries(
                    (loc_r, loc_c), (local_w, local_h), (full_w, full_h), args
                )
        ...
        obs = (
            torch.tensor(
                np.stack(
                    [np.asarray(self.res(o[0]['rgb'])).transpose(2, 0, 1) for o in obs]
                )
            )
            / 255
        ).to(device)
        ...
        for e in range(num_scenes):
            r, c = locs[e, 1], locs[e, 0]
            loc_r, loc_c = [
                int(r * 100.0 / self.map_resolution),
                int(c * 100.0 / self.map_resolution),
            ]

            local_map[e, 2:, loc_r - 1 : loc_r + 2, loc_c - 1 : loc_c + 2] = 1.0
        ...
        self.g_rollouts.extras[0].copy_(global_orientation)

        # Run Global Policy (global_goals = Long-Term Goal)
        g_value, g_action, g_action_log_prob, g_rec_states = self.g_policy.act(
        ...
        output = torch.tensor(
            np.stack(
                [
                    envs.call_at(
                        e, "get_short_term_goal", {"inputs": planner_inputs[e]}
                    )
                    for e in range(num_scenes)
                ]
            )
        ...
        explored_area_log = np.zeros((num_scenes, 25, traj_lengths))
        explored_ratio_log = np.zeros((num_scenes, 25, traj_lengths))

        total_num_steps = -1
        g_reward = torch.tensor(0)
        ...
        n_episodes = (
            self.config.TOTAL_NUM_STEPS // args.max_episode_length // num_scenes + 1
        )
        ...
        for ep_num in range(n_episodes):
            ...
            for step in range(args.max_episode_length):
                ...
                action, action_prob, local_rec_states = self.l_policy(
                    obs,
                    local_rec_states,
                    local_masks,
                    extras=local_goals,
                ...
                # ------------------------------------------------------------------
                # Env step
                step_out = envs.step(l_action.tolist())

                try:
                    observations = [s[0] for s in step_out]
                    obs = (
                        torch.tensor(
                            np.stack(
                                [
                                    np.asarray(self.res(o['rgb'])).transpose(2, 0, 1)
                ...
                except:  # New episode, after reset observations are structured differenly

                    observations = [s[0][0] for s in step_out]
                    obs = (
                        torch.tensor(
                            np.stack(
                                [
                ...
                rew = [s[1] for s in step_out]
                done = [s[2] for s in step_out]
                infos = [s[3] for s in step_out]

                l_masks = torch.FloatTensor([0 if x else 1 for x in done]).to(device)
                g_masks *= l_masks

                # save here
                for idx, obs_ep in enumerate(observations):
                    ep_step  = envs.call_at(idx, "get_step")
                    episode = envs.current_episodes()[idx]
                    paths = save_obs(self.exp_path, episode.episode_id, [obs_ep], ep_step)
                ...
                if step == self.args.max_episode_length - 1:  # Last episode step
                    init_map_and_pose()
                    del last_obs
                    last_obs = obs.detach()
                poses = (
                ...
                for e in range(num_scenes):
                    ...
                    local_map[e, 2:, loc_r - 2 : loc_r + 3, loc_c - 2 : loc_c + 3] = 1.0
                # ------------------------------------------------------------------
                # Global Policy
                if l_step == args.num_local_steps - 1:
                    ...
                    global_input[:, 4:, :, :] = nn.MaxPool2d(args.global_downscaling)(
                        full_map
                    )
                    ...
                    #     )
                    #     print(
                    #         f"Area ratio:{explored_area_log[e, ep_num, eval_g_step - 1]}"
                    #     )
                    ...
                    (
                        ...
                        self.g_rollouts.obs[g_step + 1],
                        self.g_rollouts.rec_states[g_step + 1],
                        self.g_rollouts.masks[g_step + 1],
                        extras=self.g_rollouts.extras[g_step + 1],
                        deterministic=False,
                    )
                    cpu_actions = nn.Sigmoid()(g_action).cpu().numpy()
                    global_goals = [
        ...
        return generated_observations_paths
...
class Neural_SLAM_Module(nn.Module):
    ...
    def __init__(self, args):
        ...
        pose_conv_output = self.pose_conv(
            torch.randn(1, 4, self.vision_range, self.vision_range)
        )
        ...
        self.grid_map_eval = (
            torch.zeros(args.num_processes, 2, grid_size, grid_size)
            .float()
            .to(self.device)
        )
        self.grid_map_train = (
            torch.zeros(args.slam_batch_size, 2, grid_size, grid_size)
            .float()
            .to(self.device)
        )
    ...
    def forward(
        self, obs_last, obs, poses, maps, explored, current_poses, build_maps=True
    ):
        ...
        proj1 = nn.ReLU()(self.proj1(conv_output.reshape(-1, self.conv_output_size)))
        if self.dropout > 0:
        ...
        with torch.no_grad():
            ...
            proj1 = nn.ReLU()(
                self.proj1(conv_output.reshape(-1, self.conv_output_size))
            )
            if self.dropout > 0:
                proj1 = self.dropout1(proj1)
            proj3 = nn.ReLU()(self.proj2(proj1))
            ...
            rot_mat, trans_mat = get_grid(
                st_poses, (bs, 2, grid_size, grid_size), self.device
            )
        ...
        if build_maps:
            ...
            with torch.no_grad():
                ...
                def get_new_pose_batch(pose, rel_pose_change):
                    ...
                    ) + rel_pose_change[:, 1] * torch.cos(
                        pose[:, 2] / 57.29577951308232
                    )
                    pose[:, 0] += rel_pose_change[:, 0] * torch.cos(
                        pose[:, 2] / 57.29577951308232
                    ) - rel_pose_change[:, 1] * torch.sin(
                        pose[:, 2] / 57.29577951308232
                    )
                ...
                explored2 = torch.cat(
                    (explored.unsqueeze(1), translated[:, 1:, :, :]), 1
                )
