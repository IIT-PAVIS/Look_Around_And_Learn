...
class ORBSLAM2Agent(RandomAgent):
    def __init__(self, config, device=torch.device("cuda:0")):  # noqa: B008
        ...
        # moved here by Stefano... super().__init__ is calling the reset from ORBSLAM2Agent instead if RandomAgent, why?
        super(ORBSLAM2Agent, self).__init__(config)
        ...
        self.timestep = 1.0 #0.1
        self.timing = False
        self.pose_unraveled = np.eye(4).reshape(16)
        self.trajectory_history = []

        self.reset()
        return

    # format: 4x4 rototranslation matrix, flattened by row
    def get_pose(self):
        return self.pose_unraveled

    # format: [step, timestep, 3x4 rototranslation matrix]
    def get_trajectory_history(self):
        return self.trajectory_history

    # CSV format: first row to be discarded; [step, timestep, 3x4 rototranslation matrix]
    def save_trajectory_history(self, filename='orbslam_trajectory.csv'):
    ...
    def update_internal_state(self, habitat_observation):
        ...
        if self.tracking_is_OK:
            ...
            self.pose_unraveled = self.pose6D.cpu().numpy().reshape(16)

            print("TS:", self.cur_time, "TRAJECTORY:", self.pose_unraveled)
            self.save_trajectory_history('/tmp/traj.csv')
