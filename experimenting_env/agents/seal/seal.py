...
from experimenting_env.agents.seal.model import RL_Policy_PCD
...
@baseline_registry.register_trainer(name="seal-v0")
class SEALBaseline(Baseline):
    def __init__(self, config, exp_base_dir, detectron_args, **kwargs):
        ...
        self.n_points_pcd = 100
        self.map_width, self.map_height = 400, 400
        self.visualize = False  # visualize maps
    ...
    def create_policy_inputs(self):
        inputs = []
        for idx in range(self.envs.num_envs):
            if "pcd" in self.current_observations[idx][0]:
                pcd = torch.tensor(self.current_observations[idx][0]['pcd'])
            else:
                pcd = torch.zeros(
                    (1, self.n_points_pcd, self.n_points_pcd, self.n_points_pcd)
                )


            pcd = F.avg_pool3d(pcd, 2)
            last_dim = (self.n_points_pcd - pcd.shape[-1])  
            last_dim2 = (self.n_points_pcd - pcd.shape[-2]) 
            last_dim3 = (self.n_points_pcd - pcd.shape[-3]) 
    ...
    def goto_next_subgoal(self, idx):
        if (self.got_new_plan[idx] or self.current_steps[idx] % 8 == 0) and len(
            self.sub_goals[idx]
        ) > 0:
    ...
    def predict_new_goals(self):
        if (
            ...
            rescaled_pixel_goals = [
                [
                    int(action[0] * mymaps[i].shape[1]),
                    int(action[1] * mymaps[i].shape[0]),
                ]
            ...
            for idx, rescaled_pixel_goal in enumerate(rescaled_pixel_goals):
                ...
                start_pos[0] = max(0, min(start_pos[0], scaled_down_map.shape[1] - 1))
                start_pos[1] = max(0, min(start_pos[1], scaled_down_map.shape[0] - 1))
    ...
    def train(self) -> None:
        ...
        self.g_observation_space = gym.spaces.Box(
            low=0.0,
            high=1.0,
            shape=(1, self.n_points_pcd, self.n_points_pcd, self.n_points_pcd),
            dtype='float32',
        )
        ...
        self.g_policy.load_state_dict(torch.load(self.config.ppo.load_checkpoint_path))
        ...
        while not self.is_done():
            ...
            for idx in range(self.envs.num_envs):
                ...
                if obs is None or "pcd" not in obs[0]:
                    continue
            ...
            # populate inputs
            # todo: create actual map input, modify cnn channels accordingly
            ...
            # train goal policy
            if self.current_steps[0] % self.config.ppo.num_global_steps == 0:
            ...
            if self.num_steps_done % self.config.ppo.save_periodic == 0:
                ...
                torch.save(
                    ...
                    os.path.join(
                        self.checkpoint_folder, f"checkpoint_{self.num_steps_done}"
                    ),
                )
    ...
    def generate(self) -> None:
        ...
        while not self.is_done():
            ...
            for idx in range(self.envs.num_envs):
                ...
                step = self.envs.call_at(idx, "get_episode_step")
                episode = self.envs.current_episodes()[idx]
                paths = save_obs(
                    self.exp_path, episode.episode_id, obs, step
                )
            ...
            for idx in range(self.envs.num_envs):
                if 'pcd' not in self.current_observations[idx][0]:
                    print("\n\n\nPCD NOT READY FOR IDX:",idx)
                    not_ready = True
            ...
            # orientation input
            agent_orient = [self.current_observations[idx][0]['position']['orientation'] for idx in range(self.envs.num_envs)]
            agent_orientation = [2.0 * math.acos(q.y) for q in agent_orient]
            self.global_orientation = torch.tensor(agent_orientation).reshape(self.envs.num_envs,1)
