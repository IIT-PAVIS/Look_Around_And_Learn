...
@baseline_registry.register_env(name="SemanticCuriosity-v0")
class SemanticDisagreement(BaseEnv):
    ...
    def get_and_update_semantic_curiosity_map(self):

        lower_bound, upper_bound = self._env._sim.pathfinder.get_bounds()

        disagreement_map = self.pcd.get_topdown_curiosity(lower_bound, upper_bound)
    ...
    def get_reward(self):
        ...
        if self.disagreement_map is not None:
            disagreement_amount = self.disagreement_map.sum()
