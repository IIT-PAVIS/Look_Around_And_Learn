...
@baseline_registry.register_env(name="SemanticDisagreement-sc")
class SemanticDisagreement(BaseEnv):
    def __init__(self, config, **kwargs):
        ...
        self.curiosity_map = None
        self.last_reward = 0.0
        self._episode_sum_reward = 0.0
        self.last_disagreement_amount = 0
    def is_done(self):
    ...
    def _update_pointcloud(self, observations):
        ...
        point_cloud = pu.project_semantic_masks_to_3d(
            observations[0]['depth'].squeeze(), pose, preds, infos, update_logits=False
        )
        sem_point_cloud = pu.project_semantic_masks_to_3d(
            observations[0]['depth'].squeeze(), pose, preds, infos, update_logits=True
        )
        point_cloud._episode = int(self._env.current_episode.episode_id)
        sem_point_cloud._episode = int(self._env.current_episode.episode_id)
        ...
        if self.pcd is None or self.pcd._episode != point_cloud._episode:
            ...
            self.sem_pcd = sem_point_cloud
        else:
            self.pcd += point_cloud
            sem_point_cloud.preprocess(lower_bound, upper_bound)
            self.sem_pcd += sem_point_cloud
        ...
        t = time.time()
        self.sem_pcd.preprocess(lower_bound, upper_bound)
    ...
    def get_and_update_disagreement_map(self):
        ...
        curiosity_map = self.pcd.get_topdown_curiosity(lower_bound, upper_bound)
        self.curiosity_map = curiosity_map
        disagreement_map = self.sem_pcd.get_topdown_semantic(lower_bound, upper_bound)[
            :, :, -1
        ]
    ...
    def get_reward(self):
        ...
        coefficient = 2.5 * 10 / 1000
        if self.disagreement_map is not None:
            disagreement_amount = self.curiosity_map.sum()
            disagreement_reward = coefficient*(disagreement_amount - self.last_disagreement_amount)
            self.last_disagreement_amount = disagreement_amount
