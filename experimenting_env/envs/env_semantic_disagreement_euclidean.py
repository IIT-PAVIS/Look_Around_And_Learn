...
def average_euclidean_distance(matrix):
    """
    Computes the average Euclidean distance between each pair of vectors in a matrix.

    Args:
    - matrix: a PyTorch tensor of shape (N, D), where N is the number of vectors and D is their dimensionality

    Returns:
    - a float representing the average Euclidean distance between each pair of vectors in the matrix
    """
    if matrix.ndim == 1:
        return torch.tensor(0)

    pairwise_distances = torch.cdist(matrix, matrix, p=2.0)  # compute pairwise distances between vectors
    mask = 1 - torch.eye(matrix.size(0))  # mask diagonal elements to exclude self-distances
    valid_pairwise_distances = pairwise_distances * mask  # apply mask to exclude self-distances
    num_valid_pairs = (matrix.size(0) * (matrix.size(0) - 1)) / 2  # compute number of valid pairwise distances
    average_distance = torch.sum(valid_pairwise_distances) / num_valid_pairs  # compute average distance

    return average_distance


@baseline_registry.register_env(name="SemanticDisagreement-euclidean")
class SemanticDisagreement(BaseEnv):
    def __init__(self, config, **kwargs):
        ...
        self.distance_f = average_euclidean_distance
